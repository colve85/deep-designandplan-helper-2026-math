...
const GAS_FILES = ['Code.gs', 'Stages.gs', 'UnitsCommonMath1.gs', 'UnitsCommonMath2.gs', 'UnitsExtendedMath.gs', 'UnitsOfficialMath.gs',
                   'HwpxTemplate.gs', 'Hwpx.gs', 'DocBuilder.gs', 'OperationPlan.gs', 'Api.gs'];
...
function normalizeGeminiModel(value, fallback) { const model=String(value||fallback||'gemini-2.5-flash').trim().replace(/^models\//,''); return model||fallback||'gemini-2.5-flash'; }
...
async function extractOperationPlan(ctx, req) {
  ...
  const prompt='첨부한 지난 학기 수학과 교수학습 및 평가 운영 계획 PDF를 읽고 JSON으로 구조화하세요. 현재 새 계획의 Ⅰ. 교수학습-평가 계획(schedule)은 절대 출력하거나 변경하지 않습니다. 나머지 메타데이터와 Ⅱ~Ⅲ 및 별첨 내용을 최대한 보존해 옮기세요. 날짜·시기·시수·누계는 학교마다 다르므로 추정하지 말고 빈 문자열로 두세요. 확인할 수 없는 규정도 빈 문자열로 두세요. 설명·코드블록 없이 아래 JSON 모양만 출력하세요.\n'+JSON.stringify(shape);
  try {
    ...
    const body=await res.text();if(!res.ok)return {ok:false,error:'HTTP_'+res.status,message:'PDF 분석 응답 오류('+res.status+', 모델 '+model+'). 모델명을 gemini-2.5-flash로 확인하세요.'};
